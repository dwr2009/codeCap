...
NEXUS_SurfaceHandle picdecoder_decode_source(picdecoder_t handle, const char *pictureFilename)
{
    NEXUS_SurfaceCreateSettings createSettings;
    NEXUS_PictureDecoderStartSettings pictureSettings;
    NEXUS_PictureDecoderStatus pictureStatus;
    ...
    if (pictureStatus.header.multiscan) {
        ...
        if (handle->multiScanBufferSize < minSize) {
            ...
            return picdecoder_decode_source(handle, pictureFilename);
        }
    }
...
NEXUS_SurfaceHandle picdecoder_decode_source_buffer(picdecoder_t handle, const void *pictureBuffer,int iBufferSize,int iPictureType)
{
    NEXUS_SurfaceCreateSettings createSettings;
    NEXUS_PictureDecoderStartSettings pictureSettings;
    NEXUS_PictureDecoderStatus pictureStatus;
    NEXUS_Graphics2DBlitSettings blitSettings;
    void *buffer;
    int iOffsetPicture=0,iCopySize;
    size_t size;
    int rc = -1;
    ...
    //printf("%s->%s(%d) pictureBuffer:%p iBufferSize:%d\n",__FILE__,__FUNCTION__,__LINE__,pictureBuffer,iBufferSize);
   
   
    pictureSettings.format=iPictureType;   
    pictureSettings.imageSize = iBufferSize;
    while (1) 
    {
        /* this while(1) loop is temporarily necessary for a simple virtualization. if this function fails with NEXUS_NOT_AVAILABLE,
        the underlying resource is being used. When the SID FW is rewritten for multicontext support, this loop can be removed. */
        ...
        {
            //BKNI_Sleep(1);
            BDBG_ERR(("NEXUS_PictureDecoder_GetBuffe failed"));
            goto error;
        }
        else goto error;
    }
    ...
    iCopySize=size;
    if((iCopySize+iOffsetPicture)>iBufferSize)
    {
        iCopySize=iBufferSize-iOffsetPicture;
    }    
    memcpy(buffer,((char *)pictureBuffer+iOffsetPicture),iCopySize);
    iOffsetPicture+=iCopySize;
    rc=iCopySize;
    //printf("%s->%s(%d) iCopySize:%d size:%d\n",__FILE__,__FUNCTION__,__LINE__,iCopySize,size);
    ...
    {
        if( (unsigned)(iBufferSize > size) || (pictureSettings.format == NEXUS_PictureFormat_eMpeg) )
        {
            BDBG_ERR(("Picture file size %u is larger then buffer size %u, not supported for MPEG still frames", (unsigned)iBufferSize, (unsigned)size));
            rc = -1;
            goto error;
        }
    }
    ...
    bytesRemain = pictureSettings.imageSize - iCopySize ;
    ...
    do {
        ...
        else if ( pictureStatus.state==NEXUS_PictureDecoderState_eMoreData )
        {
            ...
            {
                iCopySize=size;
                if(iOffsetPicture>=iBufferSize)
                {
                    printf("%s->%s(%d) read over\n",__FILE__,__FUNCTION__,__LINE__);
                    goto error;
                }
                
                //printf("%s->%s(%d) iCopySize:%d iOffsetPicture:%d iBufferSize:%d\n",__FILE__,__FUNCTION__,__LINE__,iCopySize,iOffsetPicture,iBufferSize);
                if((iCopySize+iOffsetPicture)>iBufferSize)
                {                    
                   iCopySize=iBufferSize-iOffsetPicture;
                }
    ...
    if (pictureStatus.header.multiscan) {
        ...
        if (handle->multiScanBufferSize < minSize) {
            ...
            return picdecoder_decode_source_buffer(handle,pictureBuffer,iBufferSize,iPictureType);
        }
#endif
    }
    //printf("%s->%s(%d) \n",__FILE__,__FUNCTION__,__LINE__);
    ...
    if ( iBufferSize > (unsigned)handle->decoderSettings.bufferSize ) {
        BDBG_MSG(("File requires multi part read, SID fw requires read from start"));
        /*fseek(fin, 0, SEEK_SET);*/
        iOffsetPicture=0;
        NEXUS_PictureDecoder_GetBuffer(handle->pictureDecoder, &buffer, &size); /* get location and size of the decoder's buffer */
        ...
        /* read file into the decoder's buffer */
        memcpy(buffer,(char *)pictureBuffer+iOffsetPicture,rc);
        iOffsetPicture=rc;
        ...
        if(rc) {
            ...
            BDBG_ERR(("failed to read in data for %s " , __FUNCTION__ ));
            rc = -1;
            goto error;
        }
    }
    ...
    do {
        ...
        else if ( pictureStatus.state==NEXUS_PictureDecoderState_eMoreData )
        {
            ...
            {
                rc=size;
                if(rc>(iBufferSize-iOffsetPicture))
                {
                    rc=iBufferSize-iOffsetPicture;
                }
                memcpy(buffer,(char *)pictureBuffer+iOffsetPicture,rc);
                iOffsetPicture=rc;
                /*rc = fread(buffer, 1, size, fin);*/                             /* read file into the decoder's buffer */
