...
# openssl
OpensslDir := $(PRJ_TOP_DIR)/BSEAV/lib/openssl


DirToTest := $(OpensslDir)/src
TestDir_Exist = $(shell test -d "$(DirToTest)";if [ $$? -eq 0 ]; then echo 1; else echo 0; fi)
ifeq ($(TestDir_Exist),1)
	ifeq ($(findstring -I$(OpensslDir)/src/include,$(CINCDIRS)),)
		CINCDIRS := $(CINCDIRS) -I$(OpensslDir)/src/include
	endif
else
	OpensslDir := 
endif
...
ifeq ($(OpensslDir),)	
    OpensslDir := $(PRJ_TOP_DIR)/obj.$(NEXUS_PLATFORM)/BSEAV/lib/openssl/arm-linux.debug
	ifeq ($(findstring -I$(OpensslDir)/include,$(CINCDIRS)),)
		CINCDIRS := $(CINCDIRS) -I$(OpensslDir)/include
	endif
endif
ifeq ($(OpensslDir),)	
    $(error Not found openssl)
endif
