...
#include "../include/MN_DMD_types.h"
...
/* Information */
DMD_ERROR_t DMD_all       (DMD_PARAMETER_t* param);
DMD_ERROR_t DMD_system    (DMD_PARAMETER_t* param);
DMD_ERROR_t DMD_lock      (DMD_PARAMETER_t* param);
DMD_ERROR_t DMD_agc       (DMD_PARAMETER_t* param);
DMD_ERROR_t DMD_ber       (DMD_PARAMETER_t* param);
DMD_ERROR_t DMD_per       (DMD_PARAMETER_t* param);
DMD_ERROR_t DMD_cnr       (DMD_PARAMETER_t* param);
DMD_ERROR_t DMD_errorfree (DMD_PARAMETER_t* param);

DMD_ERROR_t DMD_info_atsc  ( DMD_PARAMETER_t *param , DMD_u32_t id , DMD_u32_t allflag);
DMD_ERROR_t DMD_info_qam   ( DMD_PARAMETER_t *param , DMD_u32_t id , DMD_u32_t allflag);
DMD_ERROR_t DMD_info_analog( DMD_PARAMETER_t *param,DMD_u32_t id,DMD_u32_t allflag );

/* Scan ( for Channel Search )*/
DMD_ERROR_t DMD_scan_vq( DMD_PARAMETER_t *param );

DMD_ERROR_t DMD_send_regset( DMD_u32_t	devid , DMD_u8_t* regset );
...
DMD_ERROR_t DMD_device_init( DMD_PARAMETER_t* param )
{
	...
	do{
		...
		if(DMD_E_QAMB_64QAM == param->system)
		{
			iRet = DMD_send_registers( DMD_I2C_SLAVE[param->devid] , DMD_REG_QAM_B_64QAM );
		}
		else if(DMD_E_QAMB_256QAM == param->system)
		{
			iRet = DMD_send_registers( DMD_I2C_SLAVE[param->devid] , DMD_REG_QAM_B_256QAM );
		}
		else //default
		{
			iRet = DMD_send_registers( DMD_I2C_SLAVE[param->devid] , DMD_REG_ATSC );
		}
...
DMD_ERROR_t	DMD_device_set_system( DMD_PARAMETER_t* param){
	...
	default:
		//Not Support
		printf( "ERROR : Not Supported System");
		ret = DMD_E_ERROR;
	}
...
DMD_ERROR_t DMD_scan_vq( DMD_PARAMETER_t *param )
{
	...
	{
		DMD_u32_t elapsedTimeMs;
		if(now >= st)
		{
			elapsedTimeMs = now - st;
		}
		else
		{
			elapsedTimeMs = (0xFFFFFFFF - (st - now) + 1);
		}
	}
...
DMD_ERROR_t	DMD_device_set_TCB_mode( DMD_PARAMETER_t* param)
{
	DMD_ERROR_t	ret;
	DMD_u8_t	bank;
...
/* **************************************************** */
DMD_ERROR_t DMD_device_set_echo_enhance(DMD_PARAMETER_t* param, DMD_ECHO_PERFORMANCE_SET echo_out )
{    
	switch(echo_out)
       {
...
*/
/* **************************************************** */
DMD_ERROR_t DMD_device_cochan_interface_detect( DMD_PARAMETER_t *param )
{
