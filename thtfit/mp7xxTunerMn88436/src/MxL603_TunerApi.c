...
#include "../include/MxL603_TunerCfg.h"
#include "../include/MxL_Debug.h"
...
MXL_STATUS MxLWare603_API_CfgTunerMode(UINT8 devId, 
	PMXL603_TUNER_MODE_CFG_T ptunerModeCfg)
{
	UINT8 status = MXL_SUCCESS;
	MXL_STATUS mxlStatus = MXL_SUCCESS;
	UINT8 dfeRegData = 0;
	MXL603_REG_CTRL_INFO_T* tmpRegTable;
	UINT8 dfeDacIfGainVal;

	/*MxL_DLL_DEBUG0("%s: Signal Mode = %d, IF Freq = %d, xtal = %d, IF Gain = %d", 
	__FUNCTION__,
	ptunerModeCfg->signalMode,
	ptunerModeCfg->ifOutFreqinKHz,
	ptunerModeCfg->xtalFreqSel,
	ptunerModeCfg->ifOutGainLevel); */

	switch(ptunerModeCfg->signalMode)
	{
	...
	case MXL603_DIG_J83B:
		...
		if (ptunerModeCfg->ifOutFreqinKHz < HIGH_IF_35250_KHZ)
		{
			// Low power
		...
		if (ptunerModeCfg->xtalFreqSel == MXL603_XTAL_16MHz) dfeRegData = 0x0D;
		else if (ptunerModeCfg->xtalFreqSel == MXL603_XTAL_24MHz) dfeRegData = 0x0E;
		else status |= MXL_INVALID_PARAMETER;

		status |= MxLWare603_OEM_WriteRegister(devId, DFE_CSF_SS_SEL, dfeRegData);

		//by default, dfeDacIfGainVal == 0x1C
		/*
		dfeDacIfGainVal = 0x44;
		mxlStatus = MxLWare603_OEM_WriteRegister(devId, DFE_DACIF_GAIN, dfeDacIfGainVal);
		if(MXL_SUCCESS == mxlStatus)
		{
		}
		else
		{
			LOG_BLINE("I2CWrite(0x%02x,0x%02x,0x%02x) err %d\n", devId, DFE_DACIF_GAIN, dfeDacIfGainVal, mxlStatus);
		}
		//*/
	...
	case MXL603_DIG_ISDBT_ATSC:
		...
		if (MXL603_XTAL_16MHz == ptunerModeCfg->xtalFreqSel) dfeRegData = 0x0D;
		else if (MXL603_XTAL_24MHz == ptunerModeCfg->xtalFreqSel) dfeRegData = 0x0E;
		else status |= MXL_INVALID_PARAMETER;

		status |= MxLWare603_OEM_WriteRegister(devId, DFE_CSF_SS_SEL, dfeRegData);

		dfeRegData = 0;
		switch(ptunerModeCfg->ifOutGainLevel)
		{
	...
	{
		UINT8 regVal = 0;
		mxlStatus = MxLWare603_OEM_ReadRegister(devId, DFE_DACIF_GAIN, &regVal);
		if(MXL_SUCCESS == mxlStatus)
		{
			//LOG_BLINE("Mxl603,DfeDacIfGain=0x%02x\n", regVal);
		}
		else
		{
			LOG_BLINE("I2CRead(0x%02x,0x%02x) err %d\n", devId, DFE_DACIF_GAIN, mxlStatus);
		}
	}
...
MXL_STATUS MxLWare603_API_CfgTunerChanTune(UINT8 devId, 
	PMXL603_CHAN_TUNE_CFG_T pChanTuneCfg)
{
	UINT64 frequency;
	UINT32 freq = 0;
	UINT8 status = MXL_SUCCESS;
	...
	do{
		if(NULL == pChanTuneCfg)
		{
			status = (MXL_STATUS)MXL_INVALID_PARAMETER;
			break;
		}

		/*MxL_DLL_DEBUG0("%s, signal type = %d, Freq = %d, BW = %d, Xtal = %d \n",  
		__FUNCTION__,
		pChanTuneCfg->signalMode, 
		pChanTuneCfg->freqInHz, 
		pChanTuneCfg->bandWidth, 
		pChanTuneCfg->xtalFreqSel);*/

		// Abort Tune
		status |= MxLWare603_OEM_WriteRegister(devId, START_TUNE_REG, 0x00); 
		
		if (pChanTuneCfg->startTune == MXL_ENABLE)
		{	
			if (pChanTuneCfg->signalMode <= MXL603_DIG_J83B) 
			{
				// RF Frequency VCO Band Settings 
				if (pChanTuneCfg->freqInHz < 700000000) 
				{
					status |= MxLWare603_OEM_WriteRegister(devId, 0x7C, 0x1F);
					if ((pChanTuneCfg->signalMode == MXL603_DIG_DVB_C) 
						|| (pChanTuneCfg->signalMode == MXL603_DIG_J83B))
					{
						regData = 0xC1;
					}
					else
				...
				case MXL603_TERR_BW_8MHz:
					status |= MxLWare603_OEM_WriteRegister(devId, CHAN_TUNE_BW_REG, (UINT8)pChanTuneCfg->bandWidth);

					// Frequency
					frequency = pChanTuneCfg->freqInHz;
