...
static int g_iOsdWidth=1024;
static int g_iOsdHeight=768;
static int g_iX=0,g_iY=0;
static int g_iVideoWidth=1024;
static int g_iVideoHeight=768;
static NEXUS_DisplayAspectRatio g_iDisplayAspectRatio=NEXUS_DisplayAspectRatio_eAuto;
static bool g_bFullShow=false;
struct hdmi_edid 
{
	const void *data;
	unsigned size;
...
int MediaPlayer_setVidPos(media_player_t pMediaPlayer, int x, int y, int w, int h)
{
	int iNexusErr = NEXUS_SUCCESS;
	NEXUS_SurfaceComposition surfaceComposition;
    media_player_t player=pMediaPlayer;
    
    //LOG_BLINE("%s x:y(%d,%d) w:h(%d,%d)\n", __FUNCTION__,x,y,w,h);
    g_iX=x;
    g_iY=y;
    g_iVideoWidth=w;
    g_iVideoHeight=h;
    ...
    do
    {
        ...
        int osdwidth=g_iOsdWidth,osdheight=g_iOsdHeight;
        NEXUS_SurfaceComposition * pVideoSurfaceComposition;
        
        pVideoSurfaceComposition=&surfaceComposition;
        surfaceComposition.virtualDisplay.width = HwScaler_Relative_MAX_SIZE_W;
        surfaceComposition.virtualDisplay.height = HwScaler_Relative_MAX_SIZE_H;
        surfaceComposition.position.x = x;
        surfaceComposition.position.y = y;
	...
	*/
    ...
    {
        MediaPlayer_setVidWinAspectRatio(pMediaPlayer,g_iDisplayAspectRatio,g_bFullShow);
    }
...
int MediaPlayer_setVidWinAspectRatio(media_player_t pMediaPlayer, NEXUS_DisplayAspectRatio eDispAspectRatio, bool bFullVideo)
{
	...
	int x,y,width,height,osdwidth=g_iOsdWidth,osdheight=g_iOsdHeight;
	uint32_t sourceStreamWidth;	
	uint32_t sourceStreamHeight;
	int32_t monitorAS_numerator, monitorAS_denominator;
	player = pMediaPlayer;
	...
	iNexusErr=MediaPlayer_getVidPos(player,&x,&y,&width,&height);
	pVideoSurfaceComposition->virtualDisplay.width=width*osdwidth/HwScaler_Relative_MAX_SIZE_W;
	pVideoSurfaceComposition->virtualDisplay.height=height*osdheight/HwScaler_Relative_MAX_SIZE_W;
    ...
    g_iDisplayAspectRatio=eDispAspectRatio;
    g_bFullShow=bFullVideo;
	...
	else	/*not full*/
	{
        ...
        if(NEXUS_DisplayAspectRatio_eAuto == eDispAspectRatio)
        {
            monitorAS_numerator = sourceStreamWidth;
            monitorAS_denominator = sourceStreamHeight;
        }
		sourceAsPercent = pVideoSurfaceComposition->virtualDisplay.width*100/pVideoSurfaceComposition->virtualDisplay.height;
		monitorAsPercent = monitorAS_numerator*100/monitorAS_denominator;
		/*LOG("SourceAS=%d,MonitorAS=%d\n", sourceAsPercent, monitorAsPercent);	*/
		if(sourceAsPercent >= monitorAsPercent)	/*keep height*/
		{
			vidWinPos.height = pVideoSurfaceComposition->virtualDisplay.height;
			vidWinPos.y = 0;
			vidWinPos.width = ((uint32_t)vidWinPos.height*monitorAS_numerator/monitorAS_denominator);;
		...
		else	/*keep width*/			
		{
			vidWinPos.width = pVideoSurfaceComposition->virtualDisplay.width;
			vidWinPos.x = 0;
			vidWinPos.height = ((uint32_t)vidWinPos.width*monitorAS_denominator/monitorAS_numerator);
			vidWinPos.y = (pVideoSurfaceComposition->virtualDisplay.height-vidWinPos.height)/2;
		}
		//LOG_BLINE("%s vidWinPos: %d,%d,%d,%d\n", __FUNCTION__,vidWinPos.x, vidWinPos.y, vidWinPos.width, vidWinPos.height);
	} 
...
int MediaPlayer_changeHdmiDisplaySettings(media_player_t m_pMediaPlayer,int iDispParam,int iValue)
{
	...
	if(CC_LIKELY(false == bLocked))
	{
		if(CC_LIKELY(m_pMediaPlayer->hMutex))
		{
			iRet = CErrChkMutex_Lock(m_pMediaPlayer->hMutex);
			if(CC_LIKELY(ERR_SUCCESS == iRet))
			{
				bLocked = true;
			}
    ...
    if(NEXUS_MatrixCoefficients_eUnknown==hdmiOutputSettings.matrixCoefficients)
    {
        hdmiOutputSettings.matrixCoefficients=NEXUS_MatrixCoefficients_eItu_R_BT_709;
    }
	...
	case DispPARAM_ColorSpace:
		{
			if(ColorSpace_Auto == iValue)
			{
				...
				hdmiOutputSettings.colorSpace = NEXUS_ColorSpace_eAuto;
			}
			else if(ColorSpace_Rgb == iValue)
			{
				hdmiOutputSettings.colorSpace = NEXUS_ColorSpace_eRgb;
			}
			else if(ColorSpace_YCbCr422 == iValue)
			{
				hdmiOutputSettings.colorSpace = NEXUS_ColorSpace_eYCbCr422;
			}
			else if(ColorSpace_YCbCr444 == iValue)
			{
				hdmiOutputSettings.colorSpace = NEXUS_ColorSpace_eYCbCr444;
			}
			else if(ColorSpace_YCbCr420 == iValue)
			{
				hdmiOutputSettings.colorSpace = NEXUS_ColorSpace_eYCbCr420;
			}
			break;
		}
	case DispPARAM_ColorDepth:
		{
			...
			{
				hdmiOutputSettings.colorDepth = 0;
			}
			else if(ColorDepth_8bit == iValue)
			{
				hdmiOutputSettings.colorDepth = 8;
			}
			else if(ColorDepth_10bit == iValue)
			{
				hdmiOutputSettings.colorDepth = 10;
			}		
			else if(ColorDepth_12bit == iValue)
			{
				hdmiOutputSettings.colorDepth = 12;
			}
			break;
		}
	case DispPARAM_matrixcoefficients:
		{	
			if(Matrixcoefficients_Auto == iValue)//auto
			{
				hdmiOutputSettings.overrideMatrixCoefficients = false;
			}
			else if(Matrixcoefficients_Bt709 == iValue)//bt709
			{
				hdmiOutputSettings.overrideMatrixCoefficients = true;
	    		hdmiOutputSettings.matrixCoefficients = NEXUS_MatrixCoefficients_eItu_R_BT_709;
			}
			else if(Matrixcoefficients_Smpte_170M == iValue)//Smpte_170M
			{
				hdmiOutputSettings.overrideMatrixCoefficients = true;
	    		hdmiOutputSettings.matrixCoefficients = NEXUS_MatrixCoefficients_eSmpte_170M;
			}
			else if(Matrixcoefficients_BT_470_2_BG == iValue)//BT_470_2_BG
			{
				hdmiOutputSettings.overrideMatrixCoefficients = true;
	    		hdmiOutputSettings.matrixCoefficients = NEXUS_MatrixCoefficients_eItu_R_BT_470_2_BG;
			}
			else if(Matrixcoefficients_2020ncl == iValue)//2020ncl
			{
				hdmiOutputSettings.overrideMatrixCoefficients = true;
	    		hdmiOutputSettings.matrixCoefficients = NEXUS_MatrixCoefficients_eItu_R_BT_2020_NCL;
			}
			else if(Matrixcoefficients_2020cl == iValue)//2020cl
			{
				hdmiOutputSettings.overrideMatrixCoefficients = true;
	...
	case DispPARAM_Max:
		{
			NEXUS_VideoDecoderStreamInformation vidDecoderStreamInfo;
			if(!m_pMediaPlayer || !m_pMediaPlayer->videoDecoder)
			{
				iOutRet = ERROR_INVALID_PARAMETER;
				goto EXIT_PROC;
			}			

			NEXUS_SimpleVideoDecoder_GetStreamInformation(m_pMediaPlayer->videoDecoder, &vidDecoderStreamInfo);
			if(CC_LIKELY(true == vidDecoderStreamInfo.valid))
			{
				if(NEXUS_MatrixCoefficients_eUnknown==vidDecoderStreamInfo.matrixCoefficients)
                {
                    vidDecoderStreamInfo.matrixCoefficients=NEXUS_MatrixCoefficients_eItu_R_BT_709;
                }
				
				if(vidDecoderStreamInfo.matrixCoefficients != hdmiOutputSettings.matrixCoefficients)
				{
					hdmiOutputSettings.overrideMatrixCoefficients=true;
					hdmiOutputSettings.matrixCoefficients=vidDecoderStreamInfo.matrixCoefficients;
					break;
				}
			}
			goto EXIT_PROC;
	...
	//printf("[%s:%d] hdmiOutputSettings.colorSpace = %d\n",__FILE__,__LINE__,hdmiOutputSettings.colorSpace);
	//printf("[%s:%d] hdmiOutputSettings.colorDepth = %d\n",__FILE__,__LINE__,hdmiOutputSettings.colorDepth);

	iRet=NxClient_SetHdmiDisplaySettings(&hdmiOutputSettings);
	if(iRet)
	{
		iOutRet=iRet;
		goto EXIT_PROC;
...
EXIT_PROC:
	...
	if(true == bLocked)
	{
		iRet = CErrChkMutex_Unlock(m_pMediaPlayer->hMutex);
		if(ERR_SUCCESS == iRet)
		{
			bLocked = false;
		}
